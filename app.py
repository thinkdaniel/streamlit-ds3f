...
@st.cache_data
def load_data(path):
    print(f"🧑🏻‍🏫 Load data at: {datetime.now()}")
    df = pd.read_csv(path)
...
# Note: we check if the list is not empty before filtering to avoid excluding all data when no selection is made.
# We create a boolean mask to filter the DataFrame based on the selected flat types.
...
# Ensure that the date range has exactly two dates (start and end) before filtering
if len(date_range) == 2:
...
# st.columns() creates equal-width columns and returns them as a list.
...
# each col provides a .metric() method that takes a label and a value to display the metric in a visually appealing way.
...
# as_index=False keeps month as a column instead of index, which is needed for Plotly to recognize it as x-axis.
trend = (
